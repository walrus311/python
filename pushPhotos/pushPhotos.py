...
logging.basicConfig(level=logging.INFO,   #Default to INFO
                    ...
                    datefmt='%m-%d %H:%M',
                    filename='/Users/eric/code/python/pushPhotos/pushPhotos.log',
                    filemode='w')
...
yTime = datetime.now()
syncToNAS('admin','','10.0.0.12',src,'/volume1/PhotoSync/ftpuser/CameraSync/')
...
log.info("Syncing: %s" % src)
yTime = datetime.now()
syncToNAS('admin','','10.0.0.12',src,'/volume1/PhotoSync/Shared/Pictures/')
runtime = datetime.now() - yTime
log.info("Runtime: %s" % runtime)
